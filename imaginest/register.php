    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        if(isset($_POST['user']) && isset($_POST['email']) && isset($_POST['pass']) && isset($_POST['verifyPass'])){
            ...
            if (existeixUsername($userPOST)) $error = "Este usuario ya existe.";
                ...
                //PER LA PART DOS, NO FER CAS!
                //$activationCode=getActivationCode($userPOST);
                //enviarMail($emailPOST, $activationCode[0]);
                header("Location: index.php");
                exit;
                //No se como hacer para que cuando me cree el usuario y me lleve a index.php me pinte este alert
                //Supongo que deberia poner algun if(vengo de register.php --> mostrar este echo) 
                echo "<div class=alert alert-success><strong>¡Bienvenido!</strong>Usuario creado correctamente.</div>";
            }
        }
    }
?>
